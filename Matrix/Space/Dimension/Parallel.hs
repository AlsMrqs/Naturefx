...
-- Axis a = X Void
-- Axis a = X ( Line a = Point _ a _ )
-- a = Link Double [ Object = Object (Axis Link, Axis Link, Axis Link) (Double, Double, Double) ]

data Link = Link Double [Object] deriving Show

instance Eq Link where
    (==) (Link x _) (Link a _) = x == a

takeNumber :: Link -> Double
takeNumber (Link n _) = n

takeIndex :: Line Link -> Maybe Double
takeIndex Void          = Nothing
takeIndex (Point _ x _) = Just $ takeNumber x

haveObject :: Link -> Object -> Bool
haveObject (Link _ list) obj = elem obj list

insertObject :: Link -> (Object, Double) -> Link
insertObject lnk@(Link n lst) (obj, idx) = 
    if elem obj lst 
        then lnk 
        else if n == idx 
                then (Link n (obj:lst))
...
xPoint = takeIndex . takeLine_
yPoint = takeIndex . takeLine_
zPoint = takeIndex . takeLine_
